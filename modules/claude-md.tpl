...
Periodic execution system that launches a **new claude session** to run a prompt. Does not interrupt the main agent session.

- **Files:** `scripts/heartbeat/` (launch.sh, heartbeat.sh, heartbeat.conf)
...
- **Backend:** systemd timer (Linux) / launchd plist (macOS) — auto-detected

### Commands

```bash
./scripts/heartbeat/launch.sh install    # install timer
./scripts/heartbeat/launch.sh run        # activate
./scripts/heartbeat/launch.sh stop       # pause
./scripts/heartbeat/launch.sh status     # status + next run
./scripts/heartbeat/launch.sh test       # run once manually
./scripts/heartbeat/launch.sh history    # execution history
```

Additional options: `--prompt`, `--interval`, `--timeout`, `--retries`. See `launch.sh help`.
{{/if}}
